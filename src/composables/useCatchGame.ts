...
export function useCatchGame(arenaRef: Ref<HTMLElement | null>) {
    ...
    function initHearts() {
        ...
        for (let i = 0; i < HEARTS_TO_WIN; i++) {
            ...
            while (!isPositionFarEnough(pos.x, pos.y, existing, MIN_HEART_DISTANCE) && tries < MAX_POSITION_RETRIES) {
    ...
    function checkCatch() {
        ...
        const caught = hearts.value.find(
            (h) =>
                distance(cx, cy, h.x + HEART_SIZE / 2, h.y + HEART_SIZE / 2) < CATCH_RADIUS
        )
        if (caught) {
            popSound.play()
